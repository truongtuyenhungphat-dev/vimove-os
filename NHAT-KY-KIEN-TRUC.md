...
**Lưu ý quan trọng rút ra từ Phase 11**: `migrate deploy` chỉ tạo/đổi bảng — **KHÔNG**
tự gán quyền (`RolePermission`) mới cho các role đã tồn tại sẵn trong DB production.
Nếu phase mới có thêm permission mới (như `attendance.*` ở Phase 11), phải chạy thêm
`npx prisma db seed` (mục ngay dưới) **ngay sau** `migrate deploy`, nếu không user
production sẽ bị lỗi "Không có quyền truy cập" dù code đã deploy đúng — đã gặp thật
lúc deploy Phase 11, phát hiện qua browser verify sau deploy, sửa bằng cách seed lại
là hết. Luôn coi 2 lệnh `migrate deploy` + `db seed` là **1 cặp bắt buộc đi cùng nhau**
mỗi khi phase mới có permission/dữ liệu mặc định mới.
